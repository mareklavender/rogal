...
class Interpreter {
  ...
  declare(scope, name, value, tok, what) {
    if (scope.has(name)) {
      ...
      throw new RogalError(
        isBuiltin
          ? `"${name}" is a built-in action, so it can't also be a name.`
          : `"${name}" already exists.`,
        tok,
        isBuiltin
          ? { hint: `Give yours a different name.` }
          : {
  ...
  async execChange(node, scope) {
    ...
    if (t.kind === "name") {
      ...
      if (this.builtins.vars.has(t.name))
        throw new RogalError(`"${t.name}" is a built-in action, so it can't also be a name.`, node.nameTok,
          { hint: `Give yours a different name.` });
