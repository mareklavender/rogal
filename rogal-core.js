  ...
  async execChange(node, scope) {
    ...
    if (t.kind === "name") {
      ...
      if (this.builtins.vars.has(t.name))
        throw new RogalError(`"${t.name}" is already the name of a built-in action.`, node.nameTok,
          { hint: `Pick a different name, or you won't be able to use ${t.name}() any more.` });
